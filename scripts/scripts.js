...
function renderCalendar(argument1) {
    ...
    for (let i = 0; i < reinit.length; i++) {
        if (i < 8) {
            reinit[i].className = "prev-date";
        } else if (i < 39) {
            reinit[i].className = "";
        } else {
            reinit[i].className = "next-date";
        }
        // alert(reinit[i].innerHTML + " " + reinit.className);
    }
    ...
    */
    ...
    let previousMonthDays = getDaysInMonth(rightNow.getMonth() - 1, rightNow.getFullYear());
    let currentMonthDays = getDaysInMonth(rightNow.getMonth(), rightNow.getFullYear());
    ...
    if (newPointer < 35) {
        ...
        document.getElementsByClassName("calendar")[0].className = "calendar";
        while (daysUnhiddenElem.length < 42) {
